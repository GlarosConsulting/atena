...
describe('NavigateToCovenantExecutionPage', () => {
  beforeAll(async () => {
  ...
  it('should be able to navigate to covenant execution page', async () => {
    ...
    await expect(page.driver.title()).resolves.toEqual(
      'Listar Ajuste Plano Trabalho',
    );
  });

  it('should not be able to navigate to covenant execution page outside opened agreement page', async () => {
    await searchAgreements.execute({
      by: By.CNPJ,
      value: '12.198.693/0001-58',
    });
    ...
    await expect(
      navigateToCovenantExecutionPage.execute(),
    ).rejects.toBeInstanceOf(AppError);
  });

  it("should not be able to navigate to covenant execution page when not able to find 'Execução Convenente' menu", async () => {
    await searchAgreements.execute({
      by: By.CNPJ,
      value: '12.198.693/0001-58',
    });
  ...
  it("should not be able to navigate to covenant execution page when not able to find 'Ajustes do PT' submenu", async () => {
    await searchAgreements.execute({
      by: By.CNPJ,
      value: '12.198.693/0001-58',
    });
